...
// KeyToJSON converts node key to json. Returns error in case of failure.
func KeyToJSON(obj *p2p.NodeKey) ([]byte, error) {
	return cdc.MarshalJSON(obj)
}

// KeyToFile writes node key to a file with given filemode permissions.
// Written File is saved into path provided. Returns error in case of failure.
func KeyToFile(path string, perm os.FileMode, obj *p2p.NodeKey) error {
	return writeConfigIfNotExist(path, perm, obj)
}
